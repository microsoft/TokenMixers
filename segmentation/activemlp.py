...
@BACKBONES.register_module()
class ActiveTiny(ActiveMLP):
    def __init__(self, **kwargs):
        depths = [2, 3, 10, 3]
        mlp_ratios = [4, 4, 4, 4]
        embed_dims = [64, 128, 320, 512]
        share_dims = [2, 4, 4, 8]
        super(ActiveTiny, self).__init__(
            depths=depths, embed_dims=embed_dims, mlp_ratios=mlp_ratios, share_dims=share_dims, intv=2, **kwargs
        )


@BACKBONES.register_module()
class ActiveSmall(ActiveMLP):
    def __init__(self, **kwargs):
        depths = [3, 4, 18, 3]
        mlp_ratios = [8, 8, 4, 4]
        embed_dims = [64, 128, 320, 512]
        share_dims = [2, 4, 4, 8]
        super(ActiveSmall, self).__init__(
            depths=depths, embed_dims=embed_dims, mlp_ratios=mlp_ratios, share_dims=share_dims, intv=6, **kwargs
        )


@BACKBONES.register_module()
class ActiveBase(ActiveMLP):
    def __init__(self, **kwargs):
        depths = [3, 8, 27, 3]
        mlp_ratios = [8, 8, 4, 4]
        embed_dims = [64, 128, 320, 512]
        share_dims = [2, 4, 4, 8]
        super(ActiveBase, self).__init__(
            depths=depths, embed_dims=embed_dims, mlp_ratios=mlp_ratios, share_dims=share_dims, intv=6, **kwargs
        )


@BACKBONES.register_module()
class ActiveLarge(ActiveMLP):
    def __init__(self, **kwargs):
        depths = [3, 4, 24, 3]
        mlp_ratios = [4, 4, 4, 4]
        embed_dims = [96, 192, 384, 768]
        share_dims = [2, 4, 4, 8]
        super(ActiveLarge, self).__init__(
            depths=depths, embed_dims=embed_dims, mlp_ratios=mlp_ratios, share_dims=share_dims, intv=6, **kwargs
        )

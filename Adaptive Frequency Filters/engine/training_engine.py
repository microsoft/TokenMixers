...
class Trainer(object):
    """
    This class defines the training and validation code for training models with affnet
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if getattr(opts, "model.normalization.adjust_bn_momentum.enable", False):
            from affnet.layers import AdjustBatchNormMomentum
    ...
    def run(self, train_sampler=None):
        ...
        try:
            ...
            for epoch in range(self.start_epoch, max_epochs):
                ...
                if self.opts.log_wandb and self.is_master_node:
                    wandb.log({'train_loss': train_loss})
                    wandb.log({'train_top1': train_ckpt_metric})
                ...
                if self.opts.log_wandb and self.is_master_node:
                    ...
                    wandb.log({'val_top1': val_ckpt_metric})
                ...
                if self.model_ema is not None:
                    ...
                    if self.opts.log_wandb and self.is_master_node:
                        wandb.log({'val_ema_loss': val_ema_loss})
                        wandb.log({'val_ema_top1': val_ema_ckpt_metric})
                    if max_checkpoint_metric:
